...
export default function HomePage() {
  ...
  // 1. Single Auth Listener (Fixes Auth State Hook Sync Issue)
  useEffect(() => {
    const {
      data: { subscription },
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        } else if (dbFixtures && dbFixtures.length > 0) {
  ...
  // 3. Load Saved Predictions from Supabase when user is available
  useEffect(() => {
  ...
  // Allows clearing inputs without auto-resetting to 0 while typing
  ...
  const handleSaveSingle = async (fixtureId) => {
    ...
    const record = {
      ...
      pred_home_goals: pred.homeGoals === '' ? 0 : Number(pred.homeGoals) || 0,
      pred_away_goals: pred.awayGoals === '' ? 0 : Number(pred.awayGoals) || 0,
      pred_motm: pred.motm || '',
      is_submitted: true,
    };
  ...
  const handleSaveAll = async () => {
    ...
    const unsubmittedIds = fixtures
      .map((f) => f.id)
      .filter(
        (id) => !predictions[id]?.is_submitted && predictions[id]?.outcome
      );
    ...
    const records = unsubmittedIds.map((fixtureId) => ({
      ...
      pred_home_goals:
        predictions[fixtureId]?.homeGoals === ''
          ? 0
          : Number(predictions[fixtureId]?.homeGoals) || 0,
      pred_away_goals:
        predictions[fixtureId]?.awayGoals === ''
          ? 0
          : Number(predictions[fixtureId]?.awayGoals) || 0,
  ...
  const submittedCount = Object.values(predictions).filter(
    (p) => p.is_submitted
  ).length;
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center font-bold">
        Loading Premier League Predictor...
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center border-b border-slate-800 pb-4 gap-4">
          <div>
            ...
            <div className="text-xs text-slate-400">
              Exact Score: 5pts | Result: 3pts | MOTM: 2pts
            </div>
          </div>
          ...
          {user ? (
            <div className="flex items-center space-x-3 bg-slate-800 px-4 py-2 rounded-lg border border-slate-700">
              <span className="text-xs text-slate-300 font-medium">
                Logged in as:{' '}
                <strong className="text-amber-400">{user.email}</strong>
              </span>
              <button
            ...
            <form
              onSubmit={handleAuth}
              className="flex flex-wrap items-center gap-2 bg-slate-800/80 p-3 rounded-xl border border-slate-700"
            >
              <input
                type="email"
                placeholder="Email"
              ...
              {authError && (
                <div className="w-full text-red-400 text-xs mt-1">
                  {authError}
                </div>
              )}
            </form>
        ...
        <div className="bg-gradient-to-r from-purple-900 via-slate-900 to-indigo-950 p-6 md:p-8 rounded-2xl border border-purple-800/40 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="space-y-2 max-w-xl">
            ...
            <h2 className="text-2xl md:text-3xl font-black text-white">
              GAMEWEEK 1 <span className="text-amber-400">PREDICTIONS</span>
            </h2>
            <p className="text-slate-300 text-xs leading-relaxed">
              Predict match results, exact goals, and MOTM to compete on the
              global leaderboard.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-800/60 p-4 rounded-xl border border-slate-700/80 space-y-1.5">
              ...
              <p className="text-xs text-slate-400">
                Pick match outcomes, exact scores, and Man of the Match before
                kickoff.
              </p>
            </div>
            <div className="bg-slate-800/60 p-4 rounded-xl border border-slate-700/80 space-y-1.5">
              ...
              <p className="text-xs text-slate-400">
                Earn +3 pts for correct result, +5 pts for exact scoreline, and
                +2 pts for MOTM.
              </p>
            </div>
            <div className="bg-slate-800/60 p-4 rounded-xl border border-slate-700/80 space-y-1.5">
              ...
              <p className="text-xs text-slate-400">
                Points calculate post-match to determine global user rankings.
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {news.map((item) => (
              ...
              >
                ...
                <div className="p-3 pt-0 text-[10px] text-slate-500">
                  {item.time}
                </div>
              </a>
            ))}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center bg-slate-800/90 border border-slate-700 p-4 rounded-xl gap-3">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Submitted:{' '}
              <strong className="text-emerald-400">{submittedCount}</strong> /{' '}
              {fixtures.length} matches locked
            </p>
          </div>
          <button
              ...
              >
                  ...
                  <div className="flex justify-between items-center font-bold text-sm bg-slate-900/50 p-2 rounded-lg border border-slate-700/40">
                    ...
                    <div className="flex items-center space-x-1.5 mx-2">
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange(m.id, 'homeGoals', e.target.value)
                        }
                      />
                      ...
                      <input
                        ...
                        className="w-10 h-10 bg-slate-900 border border-slate-600 rounded-md text-center text-amber-400 font-bold text-base focus:outline-none focus:border-amber-400 disabled:opacity-75"
                        onChange={(e) =>
                          handleInputChange(m.id, 'awayGoals', e.target.value)
                        }
                      />
                    </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    value={motm}
                    onChange={(e) =>
                      handleInputChange(m.id, 'motm', e.target.value)
                    }
                  ...
                  >
                    <option value="">
                      -- Select{' '}
                      {activeTab === 'home' ? m.home_team : m.away_team}{' '}
                      Player --
                    </option>
                    {(activeTab === 'home'
                      ? homePlayers
                ...
                {m.final_outcome && (
                  <div className="mt-2 pt-2 border-t border-slate-700/60 bg-slate-900/60 p-2.5 rounded-xl space-y-1">
                    <div className="flex justify-between items-center text-xs font-bold text-amber-400">
                      <span>
                        Result: {m.final_home_goals} - {m.final_away_goals} (
                        {m.final_motm})
                      </span>
                      <span className="text-emerald-400">
                        +{pred.total_pts || 0} PTS
                      </span>
                    </div>
                    <div className="flex gap-1.5 text-[10px]">
                      <span className="bg-slate-800 text-slate-300 px-1.5 py-0.5 rounded">
                        Result: +{pred.points_outcome || 0}
                      </span>
                      <span className="bg-slate-800 text-slate-300 px-1.5 py-0.5 rounded">
                        Score: +{pred.points_score || 0}
                      </span>
                      <span className="bg-slate-800 text-slate-300 px-1.5 py-0.5 rounded">
